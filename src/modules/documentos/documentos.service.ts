...
export async function generateDocument(templateId: number, creditoId: number) {
  ...
  const row = dataResult.rows[0] as Record<string, unknown>;
  const variables: Record<string, string> = {
    'credito.consecutivo': String(row.consecutivo ?? ''),
    'credito.monto': money(row.val_monto_solicitado as string | number | null),
    'credito.monto_letras': `${money(row.val_monto_solicitado as string | number | null)} PESOS COLOMBIANOS`,
    'credito.plazo': String(row.num_plazo ?? ''),
    'credito.tasa': row.val_tasa ? `${row.val_tasa}%` : 'No aplica',
    'credito.cuota': money(row.val_cuota_estimada as string | number | null),
