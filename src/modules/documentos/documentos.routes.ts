...
import { z } from 'zod';
import {
  createTemplate, createTemplateVersion, documentVariables, generateDocument,
  generateFromPdfBase, getGeneratedDocument, getPdfBase, getTemplate, listPdfFields,
  listTemplates, replacePdfFields, savePdfBase
...
export async function documentosRoutes(app: FastifyInstance) {
  ...
  app.post('/plantillas/:id/pdf-base', { preHandler: [app.authenticate] }, async (request) => {
    ...
    if (!file || file.mimetype !== 'application/pdf') throw new Error('Debes seleccionar un archivo PDF');
