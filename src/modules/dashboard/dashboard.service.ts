...
export async function getDashboardGerencial(filters: DashboardFilters) {
  ...
  const [indicators, investors, states, monthly, companies, products, carteraResumen, vencida, proximos, recaudoMensual, carteraEmpresas, carteraProductos, carteraSocios, desembolsos, comitePendiente, liquidacionesPendientes, documentosPendientes, moraEdades, pendientesRol, recaudoPagaduria] = await Promise.all([
    ...
    pool.query<{ cantidad: string; valor: string; pendientes_comprobante: string }>(
      `select count(*)::int as cantidad,
       ...
       where estado_desembolso <> 'ANULADO'
         ...
         and ($2::date is null or fecha_desembolso <= $2::date)`,
      params
    ),
    pool.query<{ cantidad: string; monto: string }>(
      `select count(distinct c.id_credito)::int as cantidad,
        coalesce(sum(c.val_monto_solicitado), 0)::numeric as monto
       from "Creditos"."TBL_CREDITOS" c
       where c.v_estado_solicitud = 'COMITE_PENDIENTE'`
    ),
    pool.query<{ cantidad: string; valor_desembolso: string }>(
      `select count(*)::int as cantidad, coalesce(sum(l.valor_desembolso), 0)::numeric as valor_desembolso
    ...
    pool.query<{ cantidad: string }>(
      `select count(*)::int as cantidad
       ...
       where estado_documento not in ('APROBADO', 'VALIDADO')`
    ).catch(() => ({ rows: [{ cantidad: '0' }] })),
    pool.query<{ rango: string; cantidad: string; saldo: string }>(
      `select case
    ...
    pool.query<{ rol: string; cantidad: string; valor: string }>(
      `select case
    ...
    pool.query<{ nombre: string; valor: string; pagos: string }>(
      `select coalesce(e.v_razon_social, 'Sin empresa') as nombre,
...
export async function getReporteOperativo(filters: CarteraFilters) {
  await ensureCarteraReportColumns();
  await ensureOperationalDashboardTables();
  ...
  const [catalogos, resumen, solicitudes, desembolsos, liquidaciones, comite] = await Promise.all([
    Promise.all([
      pool.query<{ id: number; nombre: string }>('select id_empresa as id, v_razon_social as nombre from "Creditos"."TBL_EMPRESAS" order by v_razon_social'),
      pool.query<{ id: number; nombre: string }>('select id_producto_credito as id, nombre from "Creditos"."TBL_PRODUCTOS_CREDITO" order by nombre'),
      pool.query<{ estado: string }>(`select distinct coalesce(v_estado_solicitud, 'SOLICITADO') as estado from "Creditos"."TBL_CREDITOS" order by 1`)
    ]),
    pool.query<{ solicitudes: string; monto_solicitado: string; aprobadas: string; rechazadas: string; comite_pendiente: string; liquidaciones_pendientes: string; valor_liquidaciones_pendientes: string; desembolsos: string; valor_desembolsado: string }>(
      `select
    ...
    pool.query<{ credito: string; cliente: string; empresa: string; producto: string; estado: string; fecha: string; monto: string; plazo: number | null; cuota: string | null }>(
      `select c.consecutivo as credito, c.v_nombre_cliente as cliente, coalesce(e.v_razon_social, 'Sin empresa') as empresa, pc.nombre as producto,
    ...
    pool.query<{ credito: string; cliente: string; empresa: string; fecha_desembolso: string; valor_desembolso: string; banco_destino: string | null; numero_orden: string | null; estado_desembolso: string; comprobante_pago: string | null }>(
      `select c.consecutivo as credito, c.v_nombre_cliente as cliente, coalesce(e.v_razon_social, 'Sin empresa') as empresa, d.fecha_desembolso::text as fecha_desembolso,
    ...
    pool.query<{ credito: string; cliente: string; empresa: string; version: number; fecha: string; valor_desembolso: string; valor_credito: string; cuota: string }>(
      `select c.consecutivo as credito, c.v_nombre_cliente as cliente, coalesce(e.v_razon_social, 'Sin empresa') as empresa, l.numero_version as version,
    ...
    pool.query<{ credito: string; cliente: string; empresa: string; monto: string; votos: string; votos_requeridos: number | null; fecha: string }>(
      `select c.consecutivo as credito, c.v_nombre_cliente as cliente, coalesce(e.v_razon_social, 'Sin empresa') as empresa, c.val_monto_solicitado as monto,
