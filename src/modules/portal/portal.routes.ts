...
import {
  confirmPortalEmail,
  completePortalLaborProfile,
  crearSolicitudPortalCredito,
  getPortalClientById,
  listPortalCatalogs,
  listPortalCreditos,
  listPortalProductosCredito,
  loginPortalClient,
  requestPortalPasswordReset,
  registerPortalClient,
  resetPortalPassword,
  simularPortalCredito
...
export async function portalRoutes(app: FastifyInstance) {
  ...
  app.post('/registro', async (request) => {
    ...
    const token = app.jwt.sign({ portal: true, clienteId: response.cliente.id } as never);
    return {
      token,
  ...
  app.post('/login', async (request) => {
    ...
    const token = app.jwt.sign({ portal: true, clienteId: cliente.id } as never);
  ...
  app.post('/simular', { preHandler: [authenticatePortal] }, async (request) => {
    const user = request.user as unknown as PortalJwtPayload;
    const cliente = await getPortalClientById(user.clienteId);
    if (!cliente.perfilCompleto) return replyProfileRequired();
