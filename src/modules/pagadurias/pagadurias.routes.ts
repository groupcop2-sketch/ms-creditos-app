...
import {
  bulkCreateEmpleadosEmpresa,
  createEmpleadoEmpresa,
  createEmpresa,
  getEmpresa,
  listAddressCatalogs,
...
export async function pagaduriasRoutes(app: FastifyInstance) {
  app.get('/catalogos-direccion', { preHandler: [app.authenticate] }, async () =>
    listAddressCatalogs()
  );

  app.get('/catalogos-empleados', { preHandler: [app.authenticate] }, async () =>
    listEmployeeCatalogs()
  );

  app.get('/', { preHandler: [app.authenticate, requirePermission('empresas:read')] }, async () =>
    listEmpresas()
  );
