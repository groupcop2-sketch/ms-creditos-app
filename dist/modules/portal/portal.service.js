...
async function getActiveStateId() {
    const result = await pool.query('select id_estado from "Creditos"."TBL_ESTADOS" where lower(v_descripcion) = $1 limit 1', ['activo']);
...
export async function registerPortalClient(input) {
    ...
    if (duplicate.rowCount)
        throw new SecurityError('Ya existe un cliente registrado con ese correo o identificacion', 409);
...
export async function completePortalLaborProfile(clienteId, input) {
    const empresaResult = await pool.query('select id_empresa from "Creditos"."TBL_EMPRESAS" where upper(v_codigo) = upper($1) limit 1', [normalizeText(input.codigoEmpresa)]);
    if (!empresaResult.rowCount)
        throw new SecurityError('Codigo de empresa no encontrado', 404);
    const client = await pool.connect();
    try {
        await client.query('begin');
      ...
      returning id_empleado_empresa`, [empresaResult.rows[0].id_empresa, normalizeText(input.cargo), input.idTipoContrato, input.salario,
       ...
       where id_cliente_portal = $9`, [empresaResult.rows[0].id_empresa, employeeResult.rows[0].id_empleado_empresa, normalizeText(input.cargo),
...
export async function loginPortalClient(input) {
    const result = await pool.query(`${clienteSelect} where cp.v_identificacion = $1 or lower(cp.v_correo) = lower($1) limit 1`, [normalizeText(input.identificacion)]);
    if (!result.rowCount)
...
export async function getPortalClientById(clienteId) {
    const result = await pool.query(`${clienteSelect} where cp.id_cliente_portal = $1 limit 1`, [clienteId]);
    if (!result.rowCount)
        throw new SecurityError('Cliente no encontrado', 404);
    return mapCliente(result.rows[0]);
}
export async function confirmPortalEmail(input) {
    const tokenResult = await pool.query(`select id_cliente_token, id_cliente_portal
     from "Creditos"."TBL_CLIENTE_TOKENS"
     where token = $1 and tipo_token = 'CONFIRMAR_CORREO' and usado = false and fec_expira > now()
     limit 1`, [normalizeText(input.token)]);
    if (!tokenResult.rowCount)
        throw new SecurityError('Token de activacion invalido o vencido', 400);
    const client = await pool.connect();
    try {
        await client.query('begin');
        await client.query('update "Creditos"."TBL_CLIENTES_PORTAL" set ind_correo_confirmado = true, fec_actualizacion = now() where id_cliente_portal = $1', [tokenResult.rows[0].id_cliente_portal]);
        await client.query('update "Creditos"."TBL_CLIENTE_TOKENS" set usado = true where id_cliente_token = $1', [tokenResult.rows[0].id_cliente_token]);
        await client.query('commit');
    }
    catch (error) {
        await client.query('rollback');
...
export async function requestPortalPasswordReset(input) {
    const result = await pool.query(`${clienteSelect} where lower(cp.v_correo) = lower($1) limit 1`, [normalizeText(input.correo)]);
    if (!result.rowCount) {
        return { sent: true };
    }
    const cliente = mapCliente(result.rows[0]);
    const token = crypto.randomBytes(32).toString('hex');
    await pool.query(`insert into "Creditos"."TBL_CLIENTE_TOKENS" (id_cliente_portal, tipo_token, token, fec_expira)
     values ($1, 'RECUPERAR_PASSWORD', $2, now() + interval '1 hour')`, [cliente.id, token]);
...
export async function resetPortalPassword(input) {
    const tokenResult = await pool.query(`select id_cliente_token, id_cliente_portal
     ...
     limit 1`, [normalizeText(input.token)]);
    if (!tokenResult.rowCount)
        throw new SecurityError('Token invalido o vencido', 400);
    const passwordHash = await bcrypt.hash(input.password, 10);
    const client = await pool.connect();
    try {
        await client.query('begin');
        await client.query('update "Creditos"."TBL_CLIENTES_PORTAL" set v_password_hash = $1, fec_actualizacion = now() where id_cliente_portal = $2', [passwordHash, tokenResult.rows[0].id_cliente_portal]);
        await client.query('update "Creditos"."TBL_CLIENTE_TOKENS" set usado = true where id_cliente_token = $1', [tokenResult.rows[0].id_cliente_token]);
        await client.query('commit');
    }
    catch (error) {
...
export async function listPortalProductosCredito() {
    const result = await pool.query(`select p.id_producto_credito as id, p.nombre, p.descripcion, p.monto_minimo, p.monto_maximo,
...
export async function listPortalCreditos(clienteId) {
    const cliente = await getPortalClientById(clienteId);
    const result = await pool.query(`select c.id_credito, c.consecutivo, p.nombre as producto,
     ...
     order by c.fec_radicacion desc, c.id_credito desc`, [cliente.identificacion]);
...
export async function simularPortalCredito(clienteId, input) {
    await getPortalClientById(clienteId);
    if (nullableText(input.codigoVendedor))
        await findComercialByCode(input.codigoVendedor);
    return simularCredito(input);
}
export async function crearSolicitudPortalCredito(clienteId, input) {
    const cliente = await getPortalClientById(clienteId);
...
async function findComercialByCode(codigo) {
    const result = await pool.query(`select id_comercial, v_nombre_completo
     from "Creditos"."TBL_COMERCIALES"
     where upper(v_codigo_vendedor) = upper($1)
     limit 1`, [normalizeText(codigo)]);
    if (!result.rowCount)
        throw new SecurityError('El codigo del vendedor no existe', 404);
