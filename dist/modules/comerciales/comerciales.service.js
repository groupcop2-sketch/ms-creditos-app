...
async function getInactiveStateId(client) {
    const result = await client.query(`select id_estado from "Creditos"."TBL_ESTADOS"
     where lower(v_descripcion) in ('inactivo', 'inactiva')
     order by id_estado limit 1`);
    if (result.rows[0]?.id_estado)
        return result.rows[0].id_estado;
    const created = await client.query(`insert into "Creditos"."TBL_ESTADOS" (v_descripcion)
     values ('Inactivo')
     returning id_estado`);
    return created.rows[0].id_estado;
}
async function ensureAsesoresTables(client) {
    await client.query(`
...
export async function listComercialesCatalogs() {
    return withClient(async (client) => {
        ...
        const [bancos, tiposCuenta, rolesVendedor, formulas, libranzeras, generos] = await Promise.all([
            ...
            client.query('select id_rol_vendedor as id, des_rol_vendedor as nombre from "Creditos"."TBL_ROLES_VENDEDOR" order by des_rol_vendedor'),
            client.query('select id_formula_comercial as id, des_formula as nombre from "Creditos"."TBL_FORMULAS_COMERCIAL" order by des_formula'),
...
export async function listLibranzeras() {
    return withClient(async (client) => {
        await ensureAsesoresTables(client);
        const result = await client.query(`${libranzeraSelect} order by l.v_razon_social`);
        return result.rows.map(mapLibranzera);
    });
}
export async function createLibranzera(input) {
    return withClient(async (client) => {
        await ensureAsesoresTables(client);
        ...
        if (duplicate.rowCount)
            throw new SecurityError('Ya existe una libranzera con ese NIT', 409);
        const stateId = input.idEstado ?? await getActiveStateId(client);
        const created = await client.query(`insert into "Creditos"."TBL_LIBRANZERAS" (
        ...
        if (input.idBanco && input.idTipoCuenta && nullableText(input.numeroCuenta)) {
            await client.query(`insert into "Creditos"."TBL_LIBRANZERA_CUENTAS" (id_libranzera, id_banco, id_tipo_cuenta, v_num_cuenta, es_principal)
         values ($1, $2, $3, $4, true)`, [created.rows[0].id_libranzera, input.idBanco, input.idTipoCuenta, nullableText(input.numeroCuenta)]);
        }
        const result = await client.query(`${libranzeraSelect} where l.id_libranzera = $1`, [created.rows[0].id_libranzera]);
...
export async function listComerciales() {
    return withClient(async (client) => {
        await ensureAsesoresTables(client);
        const result = await client.query(`${comercialSelect} order by co.v_nombre_completo`);
        return result.rows.map(mapComercial);
    });
}
export async function createComercial(input) {
    return withClient(async (client) => {
        await ensureAsesoresTables(client);
        const exists = await client.query('select 1 from "Creditos"."TBL_LIBRANZERAS" where id_libranzera = $1 limit 1', [input.idLibranzera]);
        if (!exists.rowCount)
            throw new SecurityError('Libranzera no encontrada', 404);
        const stateId = input.idEstado ?? await getActiveStateId(client);
        const created = await client.query(`insert into "Creditos"."TBL_ASESORES" (
        ...
        const result = await client.query(`${comercialSelect} where co.id_comercial = $1`, [created.rows[0].id_comercial]);
        return mapComercial(result.rows[0]);
    });
}
export async function updateComercial(comercialId, input) {
    return withClient(async (client) => {
        await ensureAsesoresTables(client);
        const exists = await client.query('select id_asesor from "Creditos"."TBL_ASESORES" where id_asesor = $1 limit 1', [comercialId]);
        if (!exists.rowCount) {
        ...
        if (!libranzera.rowCount)
            throw new SecurityError('Libranzera no encontrada', 404);
        await client.query(`update "Creditos"."TBL_ASESORES"
       set id_libranzera = $2,
           v_identificacion = $3,
           v_primer_nombre = $4,
           v_seg_nombre = $5,
       ...
       where id_asesor = $1`, [
        ...
        const result = await client.query(`${comercialSelect} where co.id_comercial = $1`, [comercialId]);
        return mapComercial(result.rows[0]);
    });
}
export async function updateComercialEstado(comercialId, activo) {
    return withClient(async (client) => {
        await ensureAsesoresTables(client);
        const stateId = activo ? await getActiveStateId(client) : await getInactiveStateId(client);
        const updated = await client.query(`update "Creditos"."TBL_ASESORES"
       set id_estado = $2, fec_actualizacion = now()
       where id_asesor = $1
       returning id_asesor`, [comercialId, stateId]);
        if (!updated.rowCount) {
            throw new SecurityError('Este asesor no existe en TBL_ASESORES o es un registro antiguo no editable desde esta pantalla', 404);
        }
        const result = await client.query(`${comercialSelect} where co.id_comercial = $1`, [comercialId]);
