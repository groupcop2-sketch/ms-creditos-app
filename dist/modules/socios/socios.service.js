...
async function ensureInversionistasTableShape(client) {
    await client.query(`
...
async function getInversionistaEntityTypeId(client = pool) {
    ...
    const result = await client.query('select id_tipo_entidad from "Creditos"."TBL_TIPO_ENTIDADES" where lower(des_tipo_entidad) = $1 limit 1', ['inversionista']);
...
async function upsertSocioDireccion(client, socioId, input) {
    const tipoEntidadId = await getInversionistaEntityTypeId(client);
    const currentPrincipal = await client.query('select id_direccion from "Creditos"."TBL_DIRECCIONES" where id_tipo_entidad = $1 and id_entidad = $2 and coalesce(es_principal, false) = true limit 1', [tipoEntidadId, socioId]);
    if (input.esPrincipal ?? true) {
        await client.query('update "Creditos"."TBL_DIRECCIONES" set es_principal = false where id_tipo_entidad = $1 and id_entidad = $2', [tipoEntidadId, socioId]);
    }
    const values = [
        tipoEntidadId,
        socioId,
    ...
    await client.query('update "Creditos"."TBL_INVERSIONISTAS" set v_direccion = $1, id_ciudad = $2, fec_actualizacion = now() where id_inversionista = $3', [buildDireccionText(input), input.idCiudad, socioId]);
}
...
export async function listSociosCatalogs() {
    return withClient(async (client) => {
        const [tasasInversion, bancos, tiposCuenta] = await Promise.all([
            client.query(`select id_tasa_inversion as id, v_nombre as nombre, val_tasa as tasa, num_plazo as plazo
...
export async function listSocios() {
    return withClient(async (client) => {
        ...
        const result = await client.query(`${socioSelect} order by i.v_nombre_completo`);
...
export async function createSocio(input) {
    return withClient(async (client) => {
        ...
        try {
            const duplicate = await client.query('select 1 from "Creditos"."TBL_INVERSIONISTAS" where v_identificacion = $1 limit 1', [normalizeText(input.identificacion)]);
            ...
            const created = await client.query(`insert into "Creditos"."TBL_INVERSIONISTAS" (
        ...
        $15, $16
            ...
            if (input.idBanco && input.idTipoCuenta && nullableText(input.numeroCuenta)) {
                await client.query(`insert into "Creditos"."TBL_BANCO_PERSONAS" (id_banco, id_inversionista, id_tipo_cuenta, num_cuenta, fec_creacion)
         values ($1, $2, $3, $4, now())
         on conflict do nothing`, [input.idBanco, created.rows[0].id_inversionista, input.idTipoCuenta, nullableText(input.numeroCuenta)]);
            }
            const result = await client.query(`${socioSelect} where i.id_inversionista = $1`, [created.rows[0].id_inversionista]);
...
export async function updateSocio(socioId, input) {
    return withClient(async (client) => {
        await ensureCreditoFondeoTable(client);
        await ensureInversionistasTableShape(client);
        await client.query('begin');
        try {
            ...
            const duplicate = await client.query('select 1 from "Creditos"."TBL_INVERSIONISTAS" where v_identificacion = $1 and id_inversionista <> $2 limit 1', [normalizeText(input.identificacion), socioId]);
            if (duplicate.rowCount) {
                throw new SecurityError('Ya existe un socio con esa identificacion', 409);
            }
            const cityId = input.direccion?.idCiudad ?? input.idCiudad;
            ...
            await client.query(`update "Creditos"."TBL_INVERSIONISTAS" set
         ...
         where id_inversionista = $1`, [
            ...
            if (input.idBanco && input.idTipoCuenta && nullableText(input.numeroCuenta)) {
                await client.query(`insert into "Creditos"."TBL_BANCO_PERSONAS" (id_banco, id_inversionista, id_tipo_cuenta, num_cuenta, fec_creacion)
           values ($1, $2, $3, $4, now())`, [input.idBanco, socioId, input.idTipoCuenta, nullableText(input.numeroCuenta)]);
            }
            const result = await client.query(`${socioSelect} where i.id_inversionista = $1`, [socioId]);
            await client.query('commit');
            return mapSocio(result.rows[0]);
        }
        catch (error) {
...
export async function updateSocioEstado(socioId, activo) {
    return withClient(async (client) => {
        await ensureInversionistasTableShape(client);
        const stateId = activo ? await getActiveStateId(client) : await getInactiveStateId(client);
        const updated = await client.query('update "Creditos"."TBL_INVERSIONISTAS" set id_estado = $2, fec_actualizacion = now() where id_inversionista = $1 returning id_inversionista', [socioId, stateId]);
        if (!updated.rowCount) {
            throw new SecurityError('Socio no encontrado', 404);
        }
        const result = await client.query(`${socioSelect} where i.id_inversionista = $1`, [socioId]);
...
export async function listInversiones(socioId) {
    return withClient(async (client) => {
        await ensureCreditoFondeoTable(client);
        const result = await client.query(`select inv.*, e.v_descripcion as estado, tasa.v_nombre as tasa_nombre,
       ...
       order by inv.fec_inversion desc`, [socioId ?? null]);
        return result.rows.map(mapInversion);
    });
}
export async function createInversion(socioId, input) {
    return withClient(async (client) => {
        const exists = await client.query('select 1 from "Creditos"."TBL_INVERSIONISTAS" where id_inversionista = $1 limit 1', [socioId]);
        if (!exists.rowCount) {
            throw new SecurityError('Socio no encontrado', 404);
        ...
        const stateId = input.idEstado ?? await getActiveStateId(client);
        const tasa = await client.query('select id_tasa_inversion, val_tasa from "Creditos"."TBL_TASAS_INVERSION" where id_tasa_inversion = $1 limit 1', [input.idTasaInversion]);
        if (!tasa.rowCount) {
            throw new SecurityError('Tasa de inversion no encontrada', 404);
        }
        const created = await client.query(`insert into "Creditos"."TBL_INVERSIONES" (
      ...
      returning *,
        ...
        (select v_nombre from "Creditos"."TBL_TASAS_INVERSION" where id_tasa_inversion = $7) as tasa_nombre`, [input.monto, input.fechaInversion, input.plazo, Number(tasa.rows[0].val_tasa), socioId, stateId, input.idTasaInversion]);
