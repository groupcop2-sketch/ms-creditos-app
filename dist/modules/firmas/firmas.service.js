...
async function ensureFirmaTables(client) {
    await client.query(`
...
async function addFirmaEvent(client, firmaId, evento, previo, nuevo, payload) {
    await client.query(`insert into "Creditos"."TBL_CREDITO_FIRMA_EVENTOS" (
      id_credito_firma, evento, estado_anterior, estado_nuevo, payload
    ) values ($1, $2, $3, $4, $5::jsonb)`, [firmaId, evento, previo, nuevo, JSON.stringify(payload ?? {})]);
}
async function callZapSign(input) {
    ...
    const firstSigner = Array.isArray(body.signers) ? body.signers[0] : null;
    return {
...
function findPayloadValue(payload, keys) {
    ...
    for (const value of Object.values(payload)) {
        if (value && typeof value === 'object' && !Array.isArray(value)) {
            const nested = findPayloadValue(value, keys);
            if (nested)
                return nested;
        }
    }
...
function mapZapSignStatus(value) {
    const normalized = (value || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toUpperCase();
    if (['SIGNED', 'ASSINADO', 'FIRMADO', 'COMPLETED', 'DONE'].some((item) => normalized.includes(item)))
        return 'FIRMADO';
    if (['REJECTED', 'RECUSADO', 'RECHAZADO'].some((item) => normalized.includes(item)))
        return 'RECHAZADO';
    if (['CANCELED', 'CANCELLED', 'CANCELADO'].some((item) => normalized.includes(item)))
        return 'CANCELADO';
    if (['ERROR', 'FAILED'].some((item) => normalized.includes(item)))
        return 'ERROR';
    return 'ENVIADO';
}
async function downloadRemoteFile(url) {
    ...
    if (!response.ok)
        throw new SecurityError('No se pudo descargar el PDF firmado desde ZapSign', 502);
    return Buffer.from(await response.arrayBuffer());
}
export async function crearFirma(input) {
    return withClient(async (client) => {
        await client.query('begin');
        try {
            await ensureFirmaTables(client);
            const document = await client.query(`select nombre_archivo, contenido_pdf
         from "Creditos"."TBL_DOCUMENTOS_GENERADOS"
         where id_documento_generado = $1 and id_credito = $2`, [input.documentoGeneradoId, input.creditoId]);
            if (!document.rowCount)
                throw new SecurityError('Documento generado no encontrado para este credito', 404);
            const firma = await client.query(`insert into "Creditos"."TBL_CREDITO_FIRMAS" (
        ...
        returning id_credito_firma`, [
         ...
         where id_credito_firma = $4`, [zapsign.externalId, zapsign.signUrl, JSON.stringify(zapsign.raw), firmaId]);
...
export async function listFirmasCredito(creditoId) {
    return withClient(async (client) => {
        await ensureFirmaTables(client);
        const result = await client.query(`select f.id_credito_firma as id, f.id_credito as "creditoId",
       ...
       order by f.fec_creacion desc, f.id_credito_firma desc`, [creditoId]);
        return result.rows;
    });
}
export async function getFirma(firmaId) {
    const result = await pool.query(`select f.id_credito_firma as id, f.id_credito as "creditoId",
      f.id_documento_generado as "documentoGeneradoId", f.proveedor,
      f.external_document_id as "externalDocumentId", f.sign_url as "signUrl",
      f.estado, f.firmante_nombre as "firmanteNombre",
      f.firmante_correo as "firmanteCorreo", f.firmante_telefono as "firmanteTelefono",
      f.fec_envio as "fechaEnvio", f.fec_firma as "fechaFirma"
     from "Creditos"."TBL_CREDITO_FIRMAS" f where f.id_credito_firma = $1`, [firmaId]);
    if (!result.rowCount)
        throw new SecurityError('Solicitud de firma no encontrada', 404);
    return result.rows[0];
}
export async function marcarFirmaSimulada(firmaId, estado) {
    return withClient(async (client) => {
        await client.query('begin');
        try {
            await ensureFirmaTables(client);
            const current = await client.query(`select estado from "Creditos"."TBL_CREDITO_FIRMAS" where id_credito_firma = $1 for update`, [firmaId]);
            if (!current.rowCount)
                throw new SecurityError('Solicitud de firma no encontrada', 404);
            await client.query(`update "Creditos"."TBL_CREDITO_FIRMAS"
         set estado = $1, fec_firma = case when $1 = 'FIRMADO' then now() else fec_firma end,
             fec_actualizacion = now()
         where id_credito_firma = $2`, [estado, firmaId]);
...
export async function procesarWebhookZapSign(payload) {
    return withClient(async (client) => {
        await client.query('begin');
        try {
            await ensureFirmaTables(client);
            ...
            if (!externalId)
                throw new SecurityError('Webhook de ZapSign sin identificador de documento', 400);
            const current = await client.query(`select id_credito_firma, estado
         from "Creditos"."TBL_CREDITO_FIRMAS"
         where external_document_id = $1
         order by id_credito_firma desc
         limit 1
         for update`, [externalId]);
            if (!current.rowCount)
                throw new SecurityError('Firma no encontrada para el webhook recibido', 404);
         ...
         set estado = $1,
             ...
             fec_actualizacion = now()
         where id_credito_firma = $5`, [status, JSON.stringify(payload), signedPdf, signedHash, current.rows[0].id_credito_firma]);
...
export async function getPdfFirmado(firmaId) {
    return withClient(async (client) => {
        await ensureFirmaTables(client);
        const result = await client.query(`select nombre_archivo_firmado, pdf_firmado
       from "Creditos"."TBL_CREDITO_FIRMAS"
       where id_credito_firma = $1`, [firmaId]);
        if (!result.rowCount || !result.rows[0].pdf_firmado)
            throw new SecurityError('La firma no tiene PDF firmado almacenado', 404);
        return {
